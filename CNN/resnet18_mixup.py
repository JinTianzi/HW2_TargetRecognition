...
for epoch in range(num_epochs):
    ...
    for data in trainloader:
        ...
        inputs, targets_a, targets_b = Variable(inputs).cuda(),Variable(targets_a).cuda(),Variable(targets_b).cuda()
        ...
        total_right += (lam * predicted.eq(targets_a.data).cpu().sum().float()
                    + (1 - lam) * predicted.eq(targets_b.data).cpu().sum().float())
                
    print("Training accuracy for epoch {} : {}".format(epoch+1,total_right/total))
    
    if (epoch+1)%5==0:
        torch.save(model,'hw4_para_mixup.ckpt')
        
    if (epoch+1)%10==0:
        my_model = torch.load('hw4_para_mixup.ckpt')
